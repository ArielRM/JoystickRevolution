...
typedef struct {
	const unsigned char largura;
	const unsigned char altura;
	const unsigned char *pixelsAcesos;
	const unsigned char *pixelsApagados;
} figuraTransparente_t;

void cmd_LCD(uint32_t data);
void data_LCD(uint32_t data);
void inic_LCD(); 
void goto_XY(uint32_t x, uint32_t y);
void escreve2fb(const unsigned char imagem[])	;
void imprime_LCD();
...
void desenha_fig_transparente(struct  pontos_t *p, const struct figura_t *fAcesos, const struct figura_t *fApagados);
